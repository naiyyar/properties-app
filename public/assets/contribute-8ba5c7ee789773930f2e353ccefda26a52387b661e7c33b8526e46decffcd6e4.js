(function() {
  $(document).on('click', '.add_new_building', function(e) {
    var search_value;
    e.preventDefault();
    ...
    if ($("#units-search-txt").length === 1) {
      return $("#units-search-txt").remove();
    }
  });

  $(document).on('click', '#submit_review', function(e) {
    var cons, pros, residence_years, review_title, reviewer_type, score, submit;
  ...
  $(document).on('click', '#add_new_unit', function(e) {
    ...
    if ($("#unit_name").parent().parent().hasClass('hide')) {
      return $("#unit_name").parent().parent().removeClass('hide');
    }
  });

  $(document).on('click', "input[name='contribute_to']", function(e) {
    var href, next_btn, next_btn_submit, next_input, search_input, unit_search_text_box, user_id;
    ...
    if (this.value === 'unit_review' || this.value === 'unit_photos' || this.value === 'unit_amenities' || this.value === 'unit_price_history') {
      ...
      if (unit_search_text_box.length === 0) {
        return $('.unit-search').find('.form-group').append(search_input);
      }
    } else {
      $("#search-form").removeClass('hide');
      $('#buildings-search-txt').val('');
      ...
      return $('#search_item_form').attr('action', href);
    }
  });
  ...
  $(document).on('click', '#elevator', function(e) {
    ...
    if ($('#building_elevator').hasClass('hide')) {
      return $('#building_elevator').val('');
    }
  });

  $(document).on('click', '.reviewer_type', function(e) {
    var id;
    ...
    if (id === 'visitor') {
      ...
      if ($("#review_stay_time").data('validate')) {
        return $("#review_stay_time").removeAttr('data-validate');
      }
    } else {
      $("#review_stay_time").removeClass('hide');
      $("#review_stay_time").prev().show();
      return $("#review_stay_time").next().show();
    }
  });
