...
(function (global, factory) {
  ...
  function dispatchEvent(_ref) {
    ...
    // To determine featured image when index is 0
    window.draggable_elm_index = newDraggableIndex;
    
    if(oldDraggableIndex && newDraggableIndex){
    	window.data = sortable;
    }
  ...
  /** @lends Sortable.prototype */
  {
    ...
    _saveSortOrder: function _saveSortOrder(){
    	var gallery = $('#gallery');
    	var sort_order_object = [];
    	const featured_badge_class = 'fl-featured-image'
    	var children = gallery.sortable('toArray').children();
    	var badge_icon;
    	
    	children.each(function(i, j){
    		sort_order_object.push({id: j.dataset.id});
    	});
    	...
    	$.ajax({
    		...
    		dataType: 'json',
    		data: { 
    			order: sort_order_object, 
    			imageable_id: gallery.data('imageableId'),
    			imageable_type: gallery.data('imageableType'),
    		},
    		success: function(response) {
    			children.each(function(i, j){
    				badge_icon = $(j).find('.fl-featured-badge');
		    		if($(j).hasClass(featured_badge_class)){
		    			$(j).removeClass(featured_badge_class);
		    			badge_icon.html('')
		    		}
		    		if(i == 0){
		    			$(j).addClass(featured_badge_class);
		    			badge_icon.html('<h4 class="comp featured round"><span class="icon-badge font-14"></span></h4>')
		    		}
		    	});
    		}
    	})
